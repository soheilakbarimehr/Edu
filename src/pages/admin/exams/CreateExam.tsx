...
import { ArrowRight, Plus, Trash2, Image as ImageIcon } from 'lucide-react';
...
export function CreateExam() {
  ...
  const removeTopic = (index: number) => {
    const newTopics = examInfo.topics.filter((_, i) => i !== index);
    setExamInfo({
      ...examInfo,
      topics: newTopics
    });
  ...
  const updateTopic = (index: number, value: string) => {
    const newTopics = [...examInfo.topics];
  ...
  const handleSave = () => {
    const newExam = {
      ...
      topics: examInfo.topics.filter(t => t.trim() !== ''),
      questions: questions,
  ...
  const getImageOptions = () => [
    { value: IMAGES.courses.frontend, label: 'فرانت‌اند' },
    { value: IMAGES.courses.backend, label: 'بک‌اند' },
    { value: IMAGES.courses.mobile, label: 'موبایل' }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 transition-colors duration-200"
              placeholder="موضوع آزمون را وارد کنید"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">نمره قبولی (درصد)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium mb-1">سطح آزمون</label>
            <select
              value={examInfo.difficulty}
              onChange={(e) => setExamInfo({ ...examInfo, difficulty: e.target.value as 'beginner' | 'intermediate' | 'advanced' })}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 transition-colors duration-200"
            >
          ...
          <div>
            <label className="block text-sm font-medium mb-1">تصویر آزمون</label>
            <div className="flex gap-4">
              <select
                value={examInfo.image}
                onChange={(e) => setExamInfo({ ...examInfo, image: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 transition-colors duration-200"
              >
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium mb-2">سرفصل‌ها</label>
              ...
              {examInfo.topics.map((topic, index) => (
                <div key={index} className="flex gap-2">
                  <input
                    ...
                    placeholder="سرفصل را وارد کنید"
                  />
                  {examInfo.topics.length > 1 && (
                    <button
